...
export async function generateSeedSqlV3(): Promise<string> {
  ...
  const stmtKeyToId: Record<string, number> = {};
  const leanKeyToId: Record<string, number> = {};
  const eqKeyToId: Record<string, number> = {};
  const slotKeyToId: Record<string, number> = {}; // `${eqKey}_${slotName}` -> slot_id
  ...
  function getDiagramKey(item: any): string | null {
    ...
    if (name.includes('EED') || topic.includes('equation evaluator')) return 'eed';
  ...
  navRows.forEach((row, idx) => {
    ...
    navInserts.push(
      `  (${nId}, ${escapeSql(row.rawKey)}, ${appId}, ${parentId !== null ? parentId : 'NULL'}, ` +
  ...
  const formalStmts: any[] = catalog.formalStatements || [];
  const stmtSegmentPairs: { statementId: number; segmentId: number }[] = [];

  formalStmts.forEach((st: any, idx: number) => {
    const sId = idx + 1;
    ...
    statementInserts.push(
      `  (${sId}, ${escapeSql(st.key)}, ${escapeSql(scaffoldKey)}, ${escapeSql(category)}, NULL, '${type}', '${tier}', ` +
    ...
    const refSegs: string[] = st.referencedSegments || [];
  ...
  for (const [key, v] of Object.entries<any>(leanCache)) {
    leanId++;
    ...
    const matchingStmt = formalStmts.find((st: any) => st.key === key || st.scaffoldKey === key || key.startsWith(st.key));
    const stmtIdSql = matchingStmt && stmtKeyToId[matchingStmt.key] ? stmtKeyToId[matchingStmt.key].toString() : 'NULL';

    leanInserts.push(
      `  (${leanId}, ${escapeSql(key)}, ${stmtIdSql}, ${escapeSql(target)}, ${escapeSql(expr)}, ${escapeSql(sig)}, ` +
  ...
  const eqTemplateInserts: string[] = [];
  const eqSlotInserts: string[] = [];
  const eqPresetInserts: string[] = [];
  const eqPresetValInserts: string[] = [];
  ...
  for (const [eqKey, spec] of Object.entries(rawPresets)) {
    ...
    eqTemplateInserts.push(
      `  (${eqTplId}, ${escapeSql(eqKey)}, ${stmtId}, ${leanId}, ${escapeSql(spec.title)}, ` +
    ...
    spec.inputs.forEach((slot: any, sIdx: number) => {
      eqSlotId++;
      slotKeyToId[`${eqKey}_${slot.name}`] = eqSlotId;

      eqSlotInserts.push(
        `  (${eqSlotId}, ${eqTplId}, ${sIdx}, ${escapeSql(slot.name)}, ${escapeSql(slot.symbol)}, ` +
    ...
    if (spec.presets && Array.isArray(spec.presets)) {
      spec.presets.forEach((p: any) => {
        eqPresetId++;
        const pId = eqPresetId;

        eqPresetInserts.push(
          `  (${pId}, ${escapeSql(p.key)}, ${eqTplId}, ${escapeSql(p.title)}, ${escapeSql(p.display)}, ` +
        ...
        for (const [sName, sVal] of Object.entries(p.values)) {
          ...
          if (sId) {
            eqPresetValId++;
            eqPresetValInserts.push(
              `  (${eqPresetValId}, ${pId}, ${sId}, ${sVal})`
            );
          }
        }
      });
  ...
  segsList.forEach((item) => {
    ...
    while ((match = eqRegex.exec(html)) !== null) {
      ...
      segmentRefs.push(
        `  (${refId}, ${escapeSql(refKey)}, ${segId}, NULL, ${eqTplId}, 'eq', ${escapeSql(targetIdentifier)}, ${occ}, ${escapeSql(anchor)}, ${escapeSql(match[0])})`
      );
    }
    ...
    while ((match = fsdRegex.exec(html)) !== null) {
      ...
      segmentRefs.push(
        `  (${refId}, ${escapeSql(refKey)}, ${segId}, ${stmtId}, NULL, 'fsd', ${escapeSql(targetIdentifier)}, ${occ}, ${escapeSql(anchor)}, ${escapeSql(match[0])})`
      );
    }
    ...
    while ((match = ttdRegex.exec(html)) !== null) {
      ...
      segmentRefs.push(
        `  (${refId}, ${escapeSql(refKey)}, ${segId}, NULL, NULL, 'ttd', ${escapeSql(targetIdentifier)}, ${occ}, ${escapeSql(anchor)}, ${escapeSql(match[0])})`
      );
    }
  });
