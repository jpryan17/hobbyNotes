...
async function generateSeedSql() {
    ...
    try {
        const app2Index = await (0, indexUtils_js_1.loadAppMainIndex)('app2');
    ...
    navRows.forEach((row, idx) => {
        ...
        navInserts.push(`  (${nId}, ${escapeSql(row.rawKey)}, ${appId}, ${parentId !== null ? parentId : 'NULL'}, ${row.sequenceOrder}, ` +
    ...
    function visitStatement(stmt) {
        ...
        if (stmt.parentId && !visited.has(stmt.parentId)) {
            const parent = catalog.formalStatements.find((s) => s.id === stmt.parentId);
            if (parent)
                visitStatement(parent);
        }
        visited.add(stmt.id);
        orderedStatements.push(stmt);
    }
    catalog.formalStatements.forEach((s) => visitStatement(s));
    orderedStatements.forEach((stmt, idx) => {
    ...
    orderedStatements.forEach((stmt, idx) => {
        ...
        statementInserts.push(`  (${stId}, ${escapeSql(stmt.id)}, ${escapeSql(stmt.scaffoldKey)}, ${escapeSql(domainCategory)}, ` +
    ...
    catalog.calculationModes.forEach((mode, idx) => {
    ...
    catalog.calculationModes.forEach((mode, idx) => {
        ...
        modeInserts.push(`  (${mId}, ${escapeSql(mode.id)}, ${stmtId}, ${escapeSql(mode.label)}, ` +
    ...
    for (const mode of catalog.calculationModes) {
        ...
        mode.inputs.forEach((slot, idx) => {
            const defVal = Array.isArray(slot.defaultValue) ? slot.defaultValue[0] : slot.defaultValue;
            slotInserts.push(`  (${slotCounter++}, ${mId}, ${idx}, ${escapeSql(slot.name)}, ${escapeSql(slot.symbol)}, ` +
    ...
    const presetInserts = [];
    catalog.examples.forEach((ex, idx) => {
        const pId = idx + 1;
        presetKeyToId[ex.id] = pId;
        const mId = modeKeyToId[ex.modeId] || 1;
        presetInserts.push(`  (${pId}, ${escapeSql(ex.id)}, ${mId}, ` +
    ...
    for (const item of segsList) {
        ...
        while ((match = fsdRegex.exec(html)) !== null) {
            ...
            let stmtKey = null;
            if (idMatch && catalog.formalStatements.some((s) => s.id === idMatch[1])) {
                stmtKey = idMatch[1];
            }
            else if (scaffoldMatch) {
                const found = catalog.formalStatements.find((s) => s.scaffoldKey === scaffoldMatch[1]);
                if (found)
                    stmtKey = found.id;
            }
            if (stmtKey && statementKeyToId[stmtKey]) {
                ...
                const mode = catalog.calculationModes.find((m) => m.statementId === stmtKey);
        ...
        while ((match = casRegex.exec(html)) !== null) {
            ...
            let pId = null;
            if (calcId) {
                const preset = catalog.examples.find((p) => p.presetKey === calcId || p.id === calcId);
                if (preset) {
                    ...
                    const mode = catalog.calculationModes.find((m) => m.id === calcId);
                    if (mode) {
            ...
            if (stId) {
                refInserts.push(`  (${refCounter++}, ${segId}, ${stId}, ${mId !== null ? mId : 'NULL'}, ${pId !== null ? pId : 'NULL'}, 'calculator', ${occurrence++}, ${escapeSql(anchor)}, ${escapeSql(match[0])})`);
            }
        }
    }
    ...
    const leanInserts = [];
    const processedLeanKeys = new Set();
    for (const [key, entry] of Object.entries(leanCache)) {
        if (!entry || !entry.key || processedLeanKeys.has(key))
            continue;
        processedLeanKeys.add(key);
        const matchedStmt = catalog.formalStatements.find((s) => s.scaffoldKey === key || s.id === key || s.id === entry.target);
        const stId = matchedStmt ? (statementKeyToId[matchedStmt.id] || null) : null;
        leanInserts.push(`  (${escapeSql(key)}, ${stId !== null ? stId : 'NULL'}, ${escapeSql(entry.target || entry.key)}, ${escapeSql(entry.expression || '')}, ` +
    ...
    const maximaInserts = [];
    for (const [id, session] of Object.entries(maximaCache)) {
        if (!session || !session.id)
            continue;
        const middleWay = session.middleWayLink || {};
        const matchedMode = catalog.calculationModes.find((m) => m.id === session.id);
        const matchedExample = catalog.examples.find((ex) => ex.id === session.id || ex.presetKey === session.id);
        const mId = matchedMode ? modeKeyToId[matchedMode.id] : (matchedExample ? modeKeyToId[matchedExample.modeId] : null);
        maximaInserts.push(`  (${escapeSql(session.id)}, ${mId !== null ? mId : 'NULL'}, ${escapeSql(session.title)}, ${escapeSql(session.category)}, ` +
