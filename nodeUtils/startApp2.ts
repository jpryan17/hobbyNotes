...
async function start() {
  ...
  console.log('[app2R] Checking if edit-mode server is running on port 8080...');
  const running = await isServerRunning(SERVER_PORT, SERVER_HOST);

  if (running) {
    console.log('[app2R] Edit-mode server is already running.');
  } else {
    console.log('[app2R] Starting backend edit-mode server (https://localhost:8080)...');
  ...
  const cleanup = () => {
    if (serverProcess && !serverProcess.killed) {
      console.log('\n[app2R] Shutting down backend edit-mode server...');
      serverProcess.kill();
    }
    process.exit(0);
  };
