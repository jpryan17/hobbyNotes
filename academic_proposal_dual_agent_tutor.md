...
### Layer 1: The Soundness Anchor (Formal Backend Kernel)
* **Execution**: Lean 4 server running via WebAssembly / WebSocket API.
* **Role**: Evaluates student proof steps with complete logical rigor. If a step is invalid according to Lean 4's kernel, it is rejected at the formal layer.
* **Guarantee**: The system never validates an incorrect mathematical statement.

### Layer 2: The Conversational Interpreter (LLM Frontend Agent)
* **Execution**: Lightweight client-side agent (or WebGPU LLM) interfacing with the Lean 4 state.
* **Role**: When the student requests assistance (e.g., clicking the **[ Help ]** button), the frontend agent reads the current Lean 4 goal state, AST, and failed tactic attempt.
* **Translation**: Instead of displaying raw compiler errors, the agent synthesizes a human-level, Socratic hint (e.g., *"Notice that we have a conjunction in our premise. Which rule allows us to split an 'AND' statement?"*).

### Validated Intermediate Representation: The FSD Recursive AST
A central challenge in dual-agent tutoring is preventing the LLM from hallucinating when interpreting low-level compiler diagnostics. We ground this bridge in an operational, client-side semantic layer already running on `middlewaymath.app`—the **Formal Statements Demonstrator (FSD)**:
* **Recursive Syntax Tree (`evalNode`) Traces**: Instead of parsing raw compiler tracebacks, the tutoring agent inspects structured JSON evaluation trees generated by recursive evaluation across bounded domains.
* **Explicit Counterexample Grounding**: When a student's quantified expression fails (e.g., in a universal claim over bounded `ℕ_≤10`), the AST engine isolates the precise variable binding that fails:
