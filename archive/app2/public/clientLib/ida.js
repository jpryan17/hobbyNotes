...
import { createBTree } from './bTreeDiagram.js';
/**
...
export function initAnyDJSI() {
    ...
    const djsiNodes = document.querySelectorAll('.djsi[data-btree]');
    djsiNodes.forEach((node) => {
        const mode = node.getAttribute('data-btree');
...
export function initSubtree(id = 'subtree') {
    const d = createBTree(id, 'subtree');
    activeDiagrams.set(id, d);
    return d;
}
export function displaySubtree(id = 'subtree') {
    const d = activeDiagrams.get(id);
    if (d)
        d.scaleToWidth(Nav.foWidth || 900);
}
export function initCutTree(id = 'cutTree') {
    const d = createBTree(id, 'cut');
    activeDiagrams.set(id, d);
    return d;
}
export function displayCutTree(id = 'cutTree') {
    const d = activeDiagrams.get(id);
    if (d)
        d.scaleToWidth(Nav.foWidth || 900);
}
export function initOrderedtree(id = 'orderedtree') {
    const d = createBTree(id, 'order');
    activeDiagrams.set(id, d);
    return d;
}
export function displayOrderedtree(id = 'orderedtree') {
    const d = activeDiagrams.get(id);
    if (d)
        d.scaleToWidth(Nav.foWidth || 900);
}
export function initSimtree(id = 'simtree') {
    const d = createBTree(id, 'simplicity');
    activeDiagrams.set(id, d);
    return d;
}
export function displaySimtree(id = 'simtree') {
    const d = activeDiagrams.get(id);
    if (d)
        d.scaleToWidth(Nav.foWidth || 900);
}
export function initAddTree(id = 'addition') {
    const d = createBTree(id, 'addition');
    activeDiagrams.set(id, d);
    return d;
}
export function displayAddTree(id = 'addition') {
    const d = activeDiagrams.get(id);
    if (d)
        d.scaleToWidth(Nav.foWidth || 900);
}
export function initMultiplyTree(id = 'multiplication') {
    const d = createBTree(id, 'multiplication');
    activeDiagrams.set(id, d);
    return d;
}
export function displayMultiplyTree(id = 'multiplication') {
    const d = activeDiagrams.get(id);
    if (d)
        d.scaleToWidth(Nav.foWidth || 900);
}
export function initIsoTree(id = 'isomorphism') {
