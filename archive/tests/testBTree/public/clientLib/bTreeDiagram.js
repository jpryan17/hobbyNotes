import { SVGElt, SVGGrpElt, SVGText, SVGTSpan, textWidth, } from './svgElt.js';
...
export class BTreeDiagram extends SVGElt {
    ...
    constructor(config) {
        ...
        const baseMode = typeof config === 'string' ? config : config.mode;
    ...
    trimLinkCoords(bd, pos, bd1, pos1, dir = -1) {
    ...
    buildAllAntennas() {
        ...
        for (let j = 0; j < levelCount; j++) {
            const [x1, y1, xx1, yy1] = this.trimLinkCoords(this.maxBD, j, this.maxBD + 1, j * 2, -1);
            const k1 = `K${this.maxBD}${j}K${this.maxBD + 1}${j * 2}`;
            this.createLine(x1, y1, xx1, yy1, this.palette.antenna || '#c4aa7d', k1, 'antenna');
            const [x2, y2, xx2, yy2] = this.trimLinkCoords(this.maxBD, j, this.maxBD + 1, j * 2 + 1, -1);
    ...
    buildProjections() {
        ...
        for (let i = 0; i <= this.maxBD; i++) {
            for (let j = 0; j < Math.pow(2, i); j++) {
                const [x, y] = this.getNodeCenter(i, j);
                this.createLine(x, y + this.nodeSize, x, baselineY, this.palette.projectionLine || '#b0bec5', undefined, 'projection');
            }
        }
        // Baseline axis
        this.createLine(this.leftRoom, baselineY, this.width - this.rightRoom, baselineY, '#455a64', undefined, 'axis');
    }
    buildPrecisionProjections(rootNode) {
        const baselineY = this.height - 0.5 * this.bottomRoom;
    ...
    createLine(x1, y1, x2, y2, stroke, id, cls = 'link') {
        ...
        this.elt.addEventListener('click', (event) => {
            ...
            if (target && target.getAttributeNS(null, 'class') === 'node') {
                ...
                if (this.arity === 1) {
                    this.wasVisited = [key];
                    if (this.controller)
                        this.controller.onProcess(this.wasVisited);
                }
    ...
    getTreeNode(key) {
        const nodes = Array.from(this.treeGroup.children());
        return nodes.find((n) => n.getA('id') === key && n.getA('class') === 'node');
    }
    getTreeLink(key) {
        const links = Array.from(this.treeGroup.children());
        return links.find((n) => n.getA('id') === key && (n.getA('class') === 'link' || n.getA('class') === 'antenna'));
    }
    getSubtreeNodes(rootExp) {
    ...
    traceAscendingPath(targetKey, nodeColor, linkColor) {
    ...
    highlightSubtreeCone(rootKey, nodeColor, linkColor) {
    ...
    clearHighlights() {
        const elements = Array.from(this.treeGroup.children());
        elements.forEach((el) => {
 ...
 */
export function createBTree(container, config) {
