...
export class SVGGrpElt extends SVGElt {
    ...
    /*
    gmoveScale(x:number,y:number,w:number,h:number,s:number){
        this.setAA(['x',0,'y',0,'width',w,'height',h])
...
/* this is needed by tree extension
getElementByIdOrKey(idOrKey:string) {
    const id = this.tree.setId(idOrKey)
    return document.getElementById(id) as HTMLElement
}
*/
/* hopefully these can be retired
setCircle (cx:number,cy:number,radius:number,fill:string,id?:string){
    const cc = new SVGElt('circle',id)
    cc.setAA(['cx',cx,'cy',cy,'r',radius,'fill',fill,'class','node'])
    this.elt.appendChild(cc.elt)
    return cc
}
setLine(x1:number,y1:number,x2:number,y2:number,c:string,id?:string){
    const ln =  new SVGElt('line',id)
    ln.setAA(['x1',x1,'y1',y1,'x2',x2,'y2',y2,'stroke',c])
    this.elt.appendChild(ln.elt)
    return ln
}
setText (x:number,y:number,label:string,color:string){
    const text = new SVGElt('text')
    text.setAA(['x',x,'y',y,'stroke',color])
    text.setV(label)
    this.elt.appendChild(text.elt)
...
export class NavLine {
    ...
    static currentElts:[SVGElt,boolean][]=[]
    static activeElt:SVGElt|undefined
    static color={bg:'beige',std:'black',active:'mediumblue',over:'purple'}
    static margin={init:15,std:25,line:30}
    static navElt:Elt
    static mainElt:Elt
    static width:number
    static fontSize = 20
    ...
    constructor(overrides?:any[][]){
        ...
        const navSlot = document.getElementById('nav-slot') as HTMLDivElement
        NavLine.navElt = new Elt({elt:navSlot})
        NavLine.navElt.setAA(['width',NavLine.width,'height',NavLine.height,
                            'style','position:sticky;top:0'])

        NavLine.diagram = new Diagram()
        NavLine.diagram.setAA(['id','nav','width',NavLine.width,'height',NavLine.height,
                                'style',`background-color:${NavLine.color.bg};opacity:1`])
        NavLine.navElt.elt.appendChild(NavLine.diagram.elt)

        const mainSlot = document.getElementById('main-slot') as HTMLDivElement
        NavLine.mainElt = new Elt({elt:mainSlot})
        NavLine.mainElt.setA('style',`margin-top:${NavLine.height}`)
    }
    static addText(text:string,cb?:Function,active=true){
        const widget = new SVGText()
        const cbFlag = (cb)? true : false
        NavLine.currentElts.push([widget,cbFlag])
        if (active) NavLine.activeElt = widget
        widget.setV(text)
        widget.setA('font-size',NavLine.fontSize)
        if (cb){
            widget.elt.addEventListener('click',()=>{cb()})
            widget.elt.addEventListener('mouseover',()=>{widget.setA('stroke',NavLine.color.over)})
            widget.elt.addEventListener('mouseout',()=>{
                const color = (widget == NavLine.activeElt)? NavLine.color.active : NavLine.color.std
                widget.setA('stroke',color)})
        }
        if(active){
            NavLine.setActiveElt(text)
        }
        NavLine.diagram.elt.appendChild(widget.elt)
        NavLine.show()
    }
    static show(){
        let xp = NavLine.margin.init
        const yp = 1/2 * NavLine.height + 1/4 * NavLine.fontSize
        NavLine.currentElts.forEach(e => {
            const widget = e[0]
            const color = (widget == NavLine.activeElt)? NavLine.color.active : NavLine.color.std
            widget.setAA(['stroke',color,'x',xp,'y',yp])
            const w = widget.getSVGBox().width
            xp += w + NavLine.margin.std
        })
    }
    static setActiveElt(which:string){
        NavLine.currentElts.forEach(e =>{
            if(e[0].getV() == which){
                NavLine.activeElt = e[0]
                if (e[1]){
    ...
    static remove(text:string){
        NavLine.currentElts = NavLine.currentElts.filter(w => (w[0].getV() != text ))
        NavLine.show()
    }
    static clear(){
        NavLine.currentElts=[]
        NavLine.activeElt = undefined
