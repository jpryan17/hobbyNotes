...
export class TTD extends PXEParent {
    ...
    constructor() {
        ...
        this.controlsFrame.setAA(['x', x, 'y', cfY, 'height', cfH, 'fill', 'azure']);
        ...
        this.fo.setAA(['x', x, 'y', this.foY, 'style', 'background-color:white;padding:15;overflow:auto']);
        //
        this.displayButton = new SVGSelectableText(displayTable, 'display table', false);
        this.docButton = new SVGSelectableText(showDoc, 'user guide', true);
        this.proofButton = this.setProofButton();
        this.controls.push(this.displayButton);
        this.controls.push(this.docButton);
    ...
    layoutEditor() {
        ...
        if (this.usedByProofBuilderDemo) {
            this.proofButton.setAA(['pointer-events', 'auto', 'visibility', 'visible', 'stroke', 'orange']);
        }
        else {
            this.proofButton.setAA(['pointer-events', 'none', 'visibility', 'hidden']);
        }
        this.showControls();
        this.setColWidth();
    }
    ...
    setColWidth() {
        ...
        if (colCnt > 0) {
            ...
            const available = .8 * +width / colCnt;
    ...
    setProofButton() {
        ...
        proofButton.setAA(['stroke', 'orange', 'visibility', 'hidden', 'pointer-events', 'none']);
        //
    ...
    showDoc() {
        console.log(`in showDoc  docID ${this.docId}`);
        const seg = (Nav.editMode) ? Nav.segMap.get(this.docId)
            : Nav.embeddedSeg(this.docId);
        ...
        const docTable = new TTDDocTable();
        tableSlot.appendChild(docTable.elt);
    ...
    bldTree(exp) {
        ...
        let node = { nodeType: '', negations: 0, tf: false, val: '', left: undefined, right: undefined };
        if (returnType == 'split') {
            node = { nodeType: exp[posOrCnt], negations: 0, tf: false, val: '', left: undefined, right: undefined };
            ...
            if (nn[0] == PXE.lb) {
                ...
                node = { nodeType: nn, negations: posOrCnt, tf: false, val: '', left: undefined, right: undefined };
            }
        }
        return node;
    }
    ...
    bldTable() {
        ...
        hr1a.setAA(['colspan', this.predCols.length, 'headers', headers,
        ...
        hr1b.setAA(['colspan', this.expCols.length, 'headers', headers,
            'style', 'border: 1px solid black']);
        ...
        this.predCols.forEach(c => {
            ...
            h.setAA(['scope', 'col', 'width', this.colWidth, 'style', 'border: 1px solid black']);
            h.setV(c);
            hr2.append(h);
        });
        ...
        for (let i = 0; i < rowCnt; i++) {
            ...
            for (let j = 0; j < colCnt; j++) {
                ...
                if (j == this.predCols.length) {
                    ...
                    d.setAA(['id', `r${i}c${j}`, 'style', 'border: 1px solid black;text-align:center']);
                }
                else {
                    d.setAA(['id', `r${i}c${j}`, 'style',
    ...
    setHeaderListeners() {
        this.expCols.forEach((c) => {
            if (PXE.binChars.includes(c.tree.nodeType) || c.negations > 1) {
                const w = c.widget;
                w.elt.addEventListener('mouseover', (ev) => { this.handleEvent('over', ev.target); });
                w.elt.addEventListener('mouseout', (ev) => { this.handleEvent('out', ev.target); });
            }
        });
    }
    ...
    colHighlight(type, colPos) {
        ...
        if (PXE.binChars.includes(node.nodeType)) {
            if (col.negations == 0) { // handle operator
                const nl = node.left;
                const nr = node.right;
                ...
                if (type == 'over') {
                    ...
                    const cc = [[colPos, 'lavender'], [leftColPos, 'mistyrose'], [rightColPos, 'mistyrose']];
                    cc.forEach(c => { this.setColColor(c[0], c[1]); });
                }
                else {
                    ...
                    const cc = [[colPos, 'antiquewhite'], [leftColPos, 'antiquewhite'], [rightColPos, 'antiquewhite']];
                    cc.forEach(c => { this.setColColor(c[0], c[1]); });
                }
            }
            else { //handle operator negation
    ...
    expHeaderPos(node, negState) {
        ...
        for (let pos = 0; pos < this.expCols.length; pos++) {
            if (this.expCols[pos].tree == node && this.expCols[pos].negations == negState) {
                return pos;
            }
        }
    }
...
function clear() { ttd.clear(); }
function displayTable() { ttd.displayTable(); }
function showDoc() { ttd.showDoc(); }
function bldRef() { ttd.bldRef(); }
