...
export class PXE extends SVGGrpElt {
    constructor(parent) {
        ...
        this.parent.elt.addEventListener('mouseover', () => {
            document.addEventListener("keyup", PXE.handleInput);
            this.blinkId = window.setInterval(PXE.blinkCaret, 500);
        });
        this.parent.elt.addEventListener('mouseout', () => {
            document.removeEventListener("keyup", PXE.handleInput);
    ...
    layout() {
        ...
        this.txt.setAA(['x', x + PXE.fontPadding,
            'y', y + PXE.fontPadding + 2 / 3 * PXE.fontSize]);
        this.displayText();
    }
    static handleInput(e) { PXE.that.handleInput(e); }
    //
    handleInput(e) {
        const key = e.key;
        if (key == 'Backspace') {
            ...
            if (this.displayState == 'Valid') {
                const ttd = this.parent;
                ttd.backToProof();
            }
        }
        else if (PXE.charset.includes(key)) {
    ...
    setExpectClass() {
        return (PXE.predicateChars.concat(PXE.rb).includes(this.exp[this.exp.length - 1])) ? 'back' : 'front';
    }
    displayText() {
    ...
    static blinkCaret() { PXE.that.blinkCaret(); }
    blinkCaret() {
...
_a = PXE;
PXE.textFrameHeight = 30;
PXE.fontSize = 20;
PXE.fontPadding = 3;
PXE.predicateChars = 'pqrs';
PXE.negChar = 'n';
PXE.andChar = 'a';
PXE.orChar = 'o';
PXE.implyChar = 'i';
PXE.equivChar = 'e';
PXE.connectorChars = _a.andChar.concat(_a.orChar);
PXE.binChars = _a.connectorChars.concat(_a.implyChar, _a.equivChar);
PXE.lb = '[';
PXE.rb = ']';
PXE.charTokens = [['p', 0x1d45d], ['q', 0x1d45e], ['r', 0x1d45f], ['s', 0x1d460],
    ['[', 0x005b], [']', 0x005d], ['n', 0x00ac],
    ['a', 0x2227], ['o', 0x2228], ['i', 0x2192], ['e', 0x2194]];
PXE.charset = _a.charTokens.map(e => e[0]);
//# sourceMappingURL=pxe.js.map