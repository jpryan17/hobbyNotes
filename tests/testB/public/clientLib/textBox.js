import { SVGText, SVGTSpan } from './svgElt.js';
export class TBox {
    // adds (multi-line width/height shaped) tspans of text to parent...
    constructor(parent, width, height, text, includeLastLineInDiff = true, breakOnHyphen = true) {
        this.parent = parent;
        this.width = width;
        this.height = height;
        this.text = text;
        this.includeLastLineInDiff = includeLastLineInDiff;
        ...
        if (this.bpArray.length == 0) {
            ...
            if (upper == 1) {
                ...
                const possibles = this.setPossibles(upper);
    ...
    disp(possibles, evaluated, bestPos) {
    ...
    bestLineBreak(evaluated) {
        let pos = 0;
        for (let i = 1; i < evaluated.length; i++) {
            if (evaluated[i][1] < evaluated[pos][1]) {
                pos = i;
            }
        }
        return pos;
    }
    evalPossible(possible) {
        const iw = this.textWidth / (possible.length + 1);
        let diff = 0;
    ...
    isEmbeddedHyphen(pos) {
        if (!this.breakOnHyphen)
            return false;
        if (pos == 0 || pos == this.text.length - 1)
            return false;
        if (this.text[pos] != '-')
            return false;
        if ([' ', '-'].includes(this.text[pos - 1]))
            return false;
        if ([' ', '-'].includes(this.text[pos + 1]))
            return false;
        return true;
    ...
    setPossibles(upper) {
        ...
        let uppers = [];
        if (upper > 2) {
            lowers = this.setPossiblesForLineCnt(upper - 1, lowers);
        }
        if (upper > 1) {
            uppers = this.setPossiblesForLineCnt(upper, uppers);
        }
        return lowers.concat(uppers);
    }
    setPossiblesForLineCnt(lineCnt, possibles) {
        let newPossibles = [];
    ...
    setPossiblesForLineNumber(lineCnt, lineNum, possibles) {
        let newPossibles = [];
        const iw = this.textWidth / lineCnt * lineNum;
        const [before, after] = this.beforeAfterBP(iw);
        //console.log(`line cnt ${lineCnt} line num ${lineNum} iw ${iw} before ${before} after ${after}`)
    ...
    beforeAfterBP(iw) {
        ...
        for (let i = 0; i < this.bpArray.length; i++) {
            //console.log(`i ${i} this.bpArray[i][1] ${this.bpArray[i][1]} iw ${iw}`)
    ...
    maxWordWidth() {
        ...
        while (np > 0) {
            ...
            if (np == -1) {
                if (this.text.length - cp > maxSize)
                    maxSize = this.text.length - cp;
            }
            else {
                if (np - cp > maxSize)
                    maxSize = np - cp;
    ...
    nextNonSpace(cp) {
        for (let p = cp; p < this.text.length; p++)
            if (this.text[p] != ' ')
                return p;
        return -1;
    }
    subW(sub) { return this.subWH(sub)[0]; }
}
