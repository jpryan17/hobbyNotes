...
export class InteractiveTree extends Btree {
    constructor(id, w, h, maxBD, nodeSize, arity = 2, processCB, initCB, middleCB, hasPerps = false, needBeDistinct = false, btreeOverrides) {
        super(id, w, h, maxBD, nodeSize, btreeOverrides);
        this.arity = arity;
    ...
    drawCutPerps() {
        const x1 = this.x + .1 * this.width;
        const x2 = this.x + .9 * this.width;
    ...
    setNodeSelectionListeners() {
        this.elt.addEventListener('click', event => {
            ...
            console.log(`choice ${choice} this id ${this.getA('id')}`);
