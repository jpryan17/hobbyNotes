...
export class FSD extends PXEParent {
    ...
    buildStage3Controls() {
        ...
        if (hasUnquantified) {
            ...
            const dType = slot ? slot.domainType : (targetVar.startsWith("y") ? "𝒫(ℕ)" : "ℕ");
            const varChip = new SVGSelectableText(() => {
                ...
                this.renderStageControls();
                this.updatePXEText();
            }, `Var: ${targetVar}:${dType}`, true, undefined, { std: "darkred", over: "fuchsia", disabled: "grey", selected: "darkred" });
    ...
    evaluateFullStatement() {
        ...
        if (q1 === "∀" && q2 === "∀")
            return grid.every(r => r.every(Boolean));
        if (q1 === "∃" && q2 === "∃")
            return grid.some(r => r.some(Boolean));
        if (q1 === "∀" && q2 === "∃")
            return grid.every(r => r.some(Boolean));
        if (q1 === "∃" && q2 === "∀")
            return grid.some(r => r.every(Boolean));
        return grid.some(r => r.some(Boolean));
    }
