...
export class FSDRef extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.setAttribute("style", `color:${stdCol};font-weight:bold;cursor:pointer;`);
        this.addEventListener("mouseover", () => {
            this.setAttribute("style", `color:${overCol};font-weight:bold;cursor:pointer;`);
        });
        ...
        this.addEventListener("click", () => {
            ...
            const catalogStmt = FS_CATALOG.formalStatements.find((s) => s.id === scaffoldOrId || s.scaffoldKey === scaffoldOrId ||
    ...
    setupInstanceRef(instanceKey) {
        ...
        this.setAttribute("style", `color:${stdCol};font-weight:bold;cursor:pointer;text-decoration:underline;`);
        this.addEventListener("mouseover", () => {
            this.setAttribute("style", `color:${overCol};font-weight:bold;cursor:pointer;text-decoration:underline;`);
        });
        this.addEventListener("mouseout", () => {
            this.setAttribute("style", `color:${stdCol};font-weight:bold;cursor:pointer;text-decoration:underline;`);
        });
    ...
    static openScaffoldCard(scaffoldOrId, titleAttr, options) {
        const cleanId = scaffoldOrId.trim().toLowerCase();
        const matchedExample = FS_CATALOG.examples.find((ex) => (ex.presetKey && ex.presetKey.toLowerCase() === cleanId) || ex.id.toLowerCase() === cleanId);
        const matchedMode = FS_CATALOG.calculationModes.find((m) => m.id.toLowerCase() === cleanId || m.id === matchedExample?.modeId);
        const catalogStmt = FS_CATALOG.formalStatements.find((s) => s.id === scaffoldOrId || s.scaffoldKey === scaffoldOrId ||
     ...
     */
    static openInstanceCalculator(scaffoldOrId, instanceKey, titleAttr) {
        const index = Nav.indices[Nav.currentIndex];
        const choice = index ? index.choices[index.chosen] : null;
     ...
     */
    static buildInstanceCalculatorCard(scaffold, instanceKey, titleAttr, topicName) {
        const cleanKey = instanceKey.trim().toLowerCase();
        const effectiveTitle = titleAttr || "Theorem Instance Derivation";
        const isHaloMember = scaffold === "nucleus_halo_decomposition" ||
