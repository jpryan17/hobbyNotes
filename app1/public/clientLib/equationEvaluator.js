 ...
 */
function prepareJsExpr(rawExpr, diffVar = "x") {
 ...
 */
function safeEvalExpression(expr, vals, diffVar = "x") {
    try {
 ...
 */
export function inferEquationCodomain(formula, options) {
 ...
 */
export function resolveSymbolicRule(formula, options) {
    const s = (formula || "").trim();
...
export class EquationEvaluator extends Elt {
    ...
    customRhsSymbol = "y";
    customRhsDomain = "ℝ";
    multivarDiffMode = "partial";
    multivarTargetVar = "x";
    curValues = {};
    outBox;
    getSymbolicResolution() {
    ...
    renderMultivarSelector(container, symbolicRes) {
    ...
    `;
        ...
        vars.forEach(v => {
            ...
            btn.style.cssText = `padding: 5px 11px; font-size: 12px; font-weight: 700; border-radius: 4px; cursor: pointer; transition: all 0.15s ease; ${isSelected
        ...
        totalBtn.style.cssText = `padding: 5px 11px; font-size: 12px; font-weight: 700; border-radius: 4px; cursor: pointer; transition: all 0.15s ease; ${isTotalSelected
            ? "background: #0284c7; color: #ffffff; border: 1.5px solid #0284c7; box-shadow: 0 1px 3px rgba(2,132,199,0.3);"
            : "background: #ffffff; color: #0369a1; border: 1.5px solid #bae6fd;"}`;
        ...
        gradBtn.style.cssText = `padding: 5px 11px; font-size: 12px; font-weight: 700; border-radius: 4px; cursor: pointer; transition: all 0.15s ease; ${isGradSelected
            ? "background: #0284c7; color: #ffffff; border: 1.5px solid #0284c7; box-shadow: 0 1px 3px rgba(2,132,199,0.3);"
            : "background: #ffffff; color: #0369a1; border: 1.5px solid #bae6fd;"}`;
    ...
    `;
        ...
        for (const slot of this.spec.inputs) {
            for (const fn of usedFuncs) {
                if (fn.name === "ln") {
                    if (slot.min === undefined || slot.min < 0.001)
                        slot.min = 0.001;
                    ...
                    if (slot.min === undefined || slot.min < -1)
                        slot.min = -1;
                    if (slot.max === undefined || slot.max > 1)
                        slot.max = 1;
                    ...
                    if (slot.min === undefined || slot.min < 0)
                        slot.min = 0;
                    if (Number(slot.defaultValue) < 0) {
                        slot.defaultValue = 1.0;
                        this.curValues[slot.name] = 1.0;
    ...
    `;
      ...
      `;
        ...
        `;
                const pRes = probeCard.querySelector(".ee-probe-result-multivar");
                const updateMultivarProbe = () => {
                    vars.forEach(v => {
                        const inp = probeCard.querySelector(`.ee-probe-input-${v}`);
                        if (inp) {
                ...
                vars.forEach(v => {
                    const decBtn = probeCard.querySelector(`.ee-probe-dec[data-var="${v}"]`);
                    const incBtn = probeCard.querySelector(`.ee-probe-inc[data-var="${v}"]`);
                    const inp = probeCard.querySelector(`.ee-probe-input-${v}`);
        ...
        `;
                const pIn = probeCard.querySelector(".ee-probe-input");
                const pDec = probeCard.querySelector(".ee-probe-dec");
                const pInc = probeCard.querySelector(".ee-probe-inc");
                const pRes = probeCard.querySelector(".ee-probe-result");
                const updateProbe = () => {
