 ...
 */
function highlightPseudoCode(rawCode) {
    ...
    const formattedLines = lines.map((line, idx) => {
        ...
        // Highlight types
        codePart = codePart.replace(/\b(Node|Integer|Dyadic)\b/g, '<span style="color:#38bdf8;font-weight:600;">$1</span>');
        ...
        codePart = codePart.replace(/\b(SimplerOptions|Cut|ConwayAdd|Insert|EmptySet|Maximum|Minimum|EulerExp|sqr|val|node|length)\b/g, '<span style="color:#4ade80;font-weight:600;">$1</span>');
    ...
    render() {
        ...
        if (algo.example) {
            ...
            const inputsHtml = ex.inputs
                .map((inp) => `<span style="display:inline-block;background:#f1f5f9;border:1px solid #cbd5e1;padding:3px 8px;border-radius:4px;font-family:monospace;font-size:12px;color:#1e293b;margin-right:6px;">${inp}</span>`)
                .join(' ');
