...
export function parseDomainSpec(str) {
    if (!str || str === "ℕ")
        return { base: "ℕ" };
...
export class FSD extends PXEParent {
    ...
    elementVars = ["x₁", "x₂"];
    setVars = ["y₁", "y₂", "GT5", "LT10"];
    availableVars = ["x₁", "x₂", "y₁", "y₂", "GT5", "LT10"];
    selectedVar = "x₁";
    slots = [];
    varDomains = {};
    ...
    selectedColIndex = -1;
    getVarDomain(v) {
        if (this.varDomains[v])
            return this.varDomains[v];
        if (v.startsWith("y"))
            return { base: "𝒫(ℕ)" };
        return { base: "ℕ" };
    }
    ...
    cycleVarDomain(v) {
        ...
        if (cur.base === "𝒫(ℕ)")
            return;
        const otherVars = Array.from(new Set(this.slots
            .map((s) => s.assignedVar)
    ...
    buildStage2Controls() {
        ...
        varsToShow.forEach((v) => {
            ...
            }, v, this.slots.length > 0, undefined, isAssignedToActive
                ? { std: "darkred", over: "fuchsia", disabled: "grey", selected: "darkred" }
                : { std: "darkblue", over: "purple", disabled: "grey", selected: "darkblue" });
        ...
        }, isPowerSet ? "+ yVar" : "+ xVar", this.slots.length > 0);
        ...
        if (activeSlot && activeSlot.assignedVar && !activeSlot.assignedVar.startsWith("y") && activeSlot.assignedVar !== "GT5" && activeSlot.assignedVar !== "LT10") {
            ...
            const domChip = new SVGSelectableText(() => this.cycleVarDomain(activeSlot.assignedVar), domLabel, true, undefined, { std: "#0d6efd", over: "#0b5ed7", disabled: "grey", selected: "#0d6efd" });
            this.stageControls.push(domChip);
            if (typeof curDomain.param === "number") {
                const btnMinus = new SVGSelectableText(() => this.adjustVarDomainParam(activeSlot.assignedVar, -1), "-", curDomain.param > 1);
                const btnPlus = new SVGSelectableText(() => this.adjustVarDomainParam(activeSlot.assignedVar, 1), "+", curDomain.param < 64);
        ...
        const nextBtn = new SVGSelectableText(() => this.advanceStage(), hasMismatch ? `Type Clash: ${conflictingVar}` : "Quantify Variables →", allBound && !hasMismatch);
    ...
    buildStage3Controls() {
        ...
        const uniqueVars = Array.from(new Set(this.slots
            .map((s) => s.assignedVar)
        ...
        if (hasUnquantified) {
            ...
            const varChip = new SVGSelectableText(() => {
                ...
                this.renderStageControls();
                this.updatePXEText();
            }, `Var: ${targetVar}`, true, undefined, { std: "darkred", over: "fuchsia", disabled: "grey", selected: "darkred" });
            this.stageControls.push(varChip);
            const domChip = new SVGSelectableText(() => this.cycleVarDomain(targetVar), `Dom: ${dTypeStr}`, !targetVar.startsWith("y"), undefined, { std: "#0d6efd", over: "#0b5ed7", disabled: "grey", selected: "#0d6efd" });
            this.stageControls.push(domChip);
            if (typeof curDomain.param === "number") {
                const btnMinus = new SVGSelectableText(() => this.adjustVarDomainParam(targetVar, -1), "-", curDomain.param > 1);
                const btnPlus = new SVGSelectableText(() => this.adjustVarDomainParam(targetVar, 1), "+", curDomain.param < 64);
                this.stageControls.push(btnMinus);
                this.stageControls.push(btnPlus);
            }
        }
    ...
    buildStage4Controls() {
        ...
        const copyBtn = new SVGSelectableText(async () => {
    ...
    setButtonStates() {
        if (this.stage === 1) {
            ...
            if (this.buttonClear)
                this.buttonClear.setAble(this.pxe.exp.length > 0);
    ...
    setupStage2() {
        ...
        for (let i = 0; i < this.pxe.exp.length; i++) {
            ...
            if (pred) {
                pred.signature.forEach((sigDomain, slotIdx) => {
                    const domainType = sigDomain === "P_N" ? "𝒫(ℕ)" : "ℕ";
                    this.slots.push({
                        predCode: pred.code,
        ...
        this.pxe.txtFrame.elt.onclick = () => this.handlePXEClick();
        this.pxe.txt.elt.onclick = () => this.handlePXEClick();
    ...
    getVariableDomainValues(v, context, N) {
        ...
        if (!spec.filterPred)
            return allN;
        if (spec.filterPred === "EVEN")
            return allN.filter((x) => x % 2 === 0);
        if (spec.filterPred === "GT") {
            const threshold = typeof spec.param === "number" ? spec.param : (context[spec.param] ?? 0);
            return allN.filter((x) => x > threshold);
        }
        if (spec.filterPred === "LT") {
            const threshold = typeof spec.param === "number" ? spec.param : (context[spec.param] ?? (N + 1));
            return allN.filter((x) => x < threshold);
        }
        return allN;
    }
    ...
    evaluateExpCols(exp, predValues) {
        ...
        for (let i = 0; i < exp.length; i++) {
            const ch = exp[i];
            if (ch === "p")
                cols.push({ label: "GT5", val: predValues["p"] ?? true });
            else if (ch === "q")
                cols.push({ label: "LT10", val: predValues["q"] ?? true });
            else if (ch === "r")
                cols.push({ label: "GT", val: predValues["r"] ?? true });
            else if (ch === "s")
                cols.push({ label: "LT", val: predValues["s"] ?? true });
            else if (ch === "m")
