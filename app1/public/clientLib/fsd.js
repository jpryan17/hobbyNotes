...
export class FSD extends PXEParent {
    ...
    quantifierList = ["∀x", "∃y"]; // default prefix
    boundVars = {
        PG5: ["x"],
        PL10: ["x"],
        PG: ["x", "y"],
    ...
    addPred(pred) {
        ...
        if (pred === "PG5")
            this.pxe.addCharacter("p");
        else if (pred === "PL10")
            this.pxe.addCharacter("q");
        else if (pred === "PG")
            this.pxe.addCharacter("r");
        else if (pred === "PL")
            this.pxe.addCharacter("s");
    }
    layoutEditor() {
    ...
    renderBooleanMatrix(container, evalResult) {
