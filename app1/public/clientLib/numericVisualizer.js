...
export class NumericVisualizer extends Elt {
    ...
    constructor(result) {
        ...
        header.setA("style", "display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #1e293b; padding-bottom: 8px; margin-bottom: 12px; flex-wrap: wrap; gap: 8px;");
        ...
        this.auditPill.setA("style", "font-size: 11px; font-weight: 700; padding: 3px 9px; border-radius: 12px; background: #166534; color: #dcfce7; border: 1px solid #22c55e;");
        ...
        this.tensorDisplayWrap.setA("style", "margin-bottom: 12px; background: #020617; border: 1px solid #1e293b; border-radius: 6px; padding: 10px 12px;");
    ...
    buildSlotInputControls() {
        ...
        this.result.domainSlots.forEach((slot) => {
            ...
            if (slot.min !== undefined)
                input.min = slot.min.toString();
            if (slot.max !== undefined)
                input.max = slot.max.toString();
            if (slot.step !== undefined)
                input.step = slot.step.toString();
    ...
    renderTensorDisplay() {
        ...
        pre.setA("style", "margin: 0; padding: 8px 12px; background: #000000; color: #38bdf8; border: 1px solid #1e293b; border-radius: 4px; font-family: 'Fira Code', Consolas, Monaco, monospace; font-size: 12px; line-height: 1.4; overflow-x: auto;");
    ...
    renderFreeFallCanvas(frame) {
        ...
        textA.setV(`a = ${frame.data.accel.toFixed(2)}m/s² (Invariant ✓)`);
