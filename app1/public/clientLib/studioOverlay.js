...
export class StudioOverlay {
    ...
    static async openContentEditor() {
        ...
        wysiwygDiv.addEventListener('click', (e) => {
            ...
            if (target.classList.contains('fsd-chip-del') || target.closest('.fsd-chip-del')) {
                e.preventDefault();
                e.stopPropagation();
                const chip = target.closest('fsd-ref');
                if (chip) {
                    chip.remove();
                    isWysiwygDirty = true;
                    if (currentMode === 'split') {
            ...
            const fsdRef = target.closest('fsd-ref');
            if (fsdRef) {
                e.preventDefault();
                e.stopPropagation();
                ...
                StudioOverlay.openFsdInFullCanvas({
                    ...
                    if (returnedData.quantifiers)
                        fsdRef.setAttribute('quantifiers', returnedData.quantifiers);
                    else
                        fsdRef.removeAttribute('quantifiers');
                    if (returnedData.slots)
                        fsdRef.setAttribute('slots', returnedData.slots);
                    else
                        fsdRef.removeAttribute('slots');
    ...
    // =========================================================================
    static openFsdInFullCanvas(initialData, onReturn, isEditingExisting = false) {
        const contentModal = document.getElementById('studio-content-modal');
        // Temporarily hide Content Editor modal so DOM state & unsaved prose are preserved
        if (contentModal) {
            contentModal.style.display = 'none';
