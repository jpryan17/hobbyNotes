 ...
 */
export class SubtreeController implements IBTreeController {
  private leftNodes: string[] = [];
  private rightNodes: string[] = [];
 ...
 */
export class SimplicityController implements IBTreeController {
  constructor(private diagram: BTreeDiagram) {}
  ...
  onProcess(visited: string[]): void {
    ...
    const formatSet = (ids: string[]) =>
      '{' + ids.map((id) => setVal(setExp(id)).trim()).join(', ') + '}';
  ...
  private getSimplerNodeIds(key: string): string[] {
 ...
 */
export class OrderController implements IBTreeController {
  constructor(private diagram: BTreeDiagram) {}
  ...
  private colorSubtree(key: string, side: '-' | '+', color: string): void {
  ...
  private getSimplerNodes(nodeExp: string) {
    const all: string[] = [];
    const left: string[] = [];
    const right: string[] = [];
 ...
 */
export class CutController implements IBTreeController {
  constructor(private diagram: BTreeDiagram) {}
  ...
  onProcess(visited: string[]): void {
    ...
    const [leftKey, leftExp, rightKey, rightExp] =
      compareExps(expA, expB) === 1
        ? [keyB, expB, keyA, expA]
        : [keyA, expA, keyB, expB];
...
export class OpController implements IBTreeController {
  ...
  onProcess(visited: string[]): void {
    ...
    const resultExp =
      this.op === '+' ? RsOps.add(exp1, exp2) : RsOps.multiply(exp1, exp2);
 ...
 */
export class IsoController implements IBTreeController {
  private currentOp: '+' | '\u2217' = '+';
 ...
 */
export class OmegaStateController implements IBTreeController {
  constructor(private diagram: BTreeDiagram) {}
