...
export class PseudoRef extends HTMLElement {
  ...
  connectedCallback(): void {
    ...
    this.setAttribute(
      'style',
      `display:inline-block;color:${PseudoRef.stdColor};font-weight:bold;cursor:pointer;text-decoration:underline;text-decoration-color:${PseudoRef.borderColor};text-underline-offset:3px;padding:2px 8px;border-radius:4px;background:transparent;transition:background 0.15s, color 0.15s;`
    );

    this.addEventListener('mouseover', () => {
      this.setAttribute(
        'style',
        `display:inline-block;color:${PseudoRef.overColor};font-weight:bold;cursor:pointer;text-decoration:underline;text-decoration-color:${PseudoRef.stdColor};text-underline-offset:3px;background:${PseudoRef.bgColor};padding:2px 8px;border-radius:4px;box-shadow:0 1px 2px rgba(0,0,0,0.05);`
      );
    });
    ...
    this.addEventListener('mouseout', () => {
      this.setAttribute(
        'style',
        `display:inline-block;color:${PseudoRef.stdColor};font-weight:bold;cursor:pointer;text-decoration:underline;text-decoration-color:${PseudoRef.borderColor};text-underline-offset:3px;background:transparent;padding:2px 8px;border-radius:4px;`
      );
    });
