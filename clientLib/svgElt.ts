...
export class SVGElt extends Elt {
    constructor(qname:string, id?:string){
        super(qname,id,'S')
    }
    getBB(){
        const ge = this.elt as SVGGraphicsElement
        return ge.getBBox()
    }
    move(x:number,y:number){
        this.setAA(['x',0,'y',0,'transform',`translate(${x},${y})`])
    }
    scale(s:number,sy?:number){
    ...
    scaleMove(sx:number,sy:number,x:number,y:number){
        this.setAA(['x',0,'y',0])
...
export class SVGGrpElt extends SVGElt {
    ...
    gmove (x:number,y:number){
        this.setAA(['x',0,'y',0])
        this.setA('transform',`translate(${x},${y})`)
    }
    xscale(s:number,sy?:number){
        if(sy){
            this.setA('transform',`scale(${s} ${sy})`)
        } else {
            this.setA('transform',`scale(${s})`)
    ...
    gmoveScale(x:number,y:number,s:number){
        this.setAA(['x',0,'y',0])
        this.setA('transform',`scale(${s}) translate(${x},${y})`)
    }
    gmoveScales(x:number,y:number,sx:number,sy:number){
...
export class SVGTextElt extends SVGElt{
    constructor(qparms:string,id?:string){
        super(qparms,id)
    }
    static clearText(elt:SVGElement){
    ...
    clear(){
        let textElt = this.elt as SVGElement
        SVGTextElt.clearText(textElt)
    }
    getEstimatedMaxWidth(maxTextLen:number){
        const clone = this.elt.cloneNode(true) as SVGGraphicsElement
        const slot = document.getElementById('scratch-slot') as HTMLElement
        slot.innerHTML = ''
    ...
    getTextWidthHeight(){
        //const clone = this.elt.cloneNode(true) as SVGGraphicsElement
        const clone = this.elt as SVGGraphicsElement
        const slot = document.getElementById('scratch-slot') as HTMLElement
        slot.innerHTML = ''
        const svg = new SVGElt('svg')
...
export class SVGTSpan extends SVGTextElt{
    parent:SVGTextElt
    constructor(parent:SVGTextElt, x?:number,y ?:number, id?:string,){
        super('tspan',id)
        this.parent=parent
...
export interface SelectableTextColors{
    std:string, over:string, disabled:string, selected:string
}
export class SVGSelectableText extends SVGText {
    color:SelectableTextColors={std:'darkblue',over:'purple',disabled:'grey',selected:'darkred'}

    constructor(cb:Function, text:string, enabled=true, id?:string,color?:SelectableTextColors) {
        super(id)
        ...
        this.elt.addEventListener('mouseout',()=>{
            ...
            if(enabled=='all') this.setA('stroke',this.color.std)
        })
    }
    setAble(enabled:boolean){
        if(enabled){
...
export class SVGDiagram extends SVGElt {
    constructor(public group:SVGGrpElt){
        super('svg')
...
export class SelectableTextList extends SVGElt{
    ...
    choicePos!:number
    //
    constructor(public cb:Function, stl?:STL){
        super('svg')
        if(stl){
            if(stl.maxDisplayLines) this.maxDisplayLines = stl.maxDisplayLines
            if(stl.maxLineCharCnt) this.maxLineCharCnt = stl.maxLineCharCnt
            if(stl.fontSize) this.fontSize = stl.fontSize
            if(stl.bgColor) this.bgColor = stl.bgColor
            if(stl.baseColor) this.baseColor = stl.baseColor
            if(stl.overColor) this.overColor = stl.overColor
            if(stl.frameColor) this.frameColor = stl.frameColor
            if(stl.frameWidth) this.frameWidth = stl.frameWidth
            if(stl.id) this.id = stl.id
        }
    ...
    setList(inList?:SVGText[]){
        if (inList) this.list = inList
        this.vp = 0
        ...
        this.list.forEach((e,p)=>{
            ...
            e.elt.addEventListener('click',(ev)=>{
                ...
                const line = Elt.wrapper(target) as SVGElt
                this.choicePos = p
