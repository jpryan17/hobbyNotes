import { MAXIMA_CACHE, MaximaCacheEntry } from './maximaCache.js';

export class StemCard extends HTMLElement {
  private entryId: string = 'heat_diffusion_1d';
  private entry?: MaximaCacheEntry;
  private currentTab: 'simulation' | 'maxima' | 'fourier' = 'simulation';
  ...
  private setTab(tab: 'simulation' | 'maxima' | 'fourier') {
    this.currentTab = tab;
    this.render();
  }
  ...
  private renderSimulationTab(): string {
    const sim = this.entry?.numericalSimulation;
    if (!sim) return '<p>No simulation data available.</p>';
  ...
  private generateSVG(sim: NonNullable<MaximaCacheEntry['numericalSimulation']>, timeIdx: number): string {
    const width = 640;
    const height = 220;
    ...
    // Coordinate mapping
    const getX = (nodeIdx: number) => padding.left + (nodeIdx / (sim.spatialNodes.length - 1)) * plotWidth;
    const getY = (u: number) => padding.top + plotHeight - ((u - minU) / (maxU - minU)) * plotHeight;
  ...
  private renderMaximaTab(): string {
  ...
  private renderFourierTab(): string {
  ...
  private attachEventListeners() {
    ...
    const slider = this.querySelector('#timeSlider') as HTMLInputElement | null;
    if (slider) {
