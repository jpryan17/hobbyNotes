...
export class EquationEvaluator extends Elt {
  ...
  private customDomain: string = "ℝ";
  private customRhsSymbol: string = "y";
  private curValues: Record<string, number> = {};
  private outBox!: HTMLDivElement;
  private controlsGrid!: HTMLDivElement;
  private contentContainer!: HTMLDivElement;

  constructor(initialPresetOrSpec?: string | EquationSpec) {
    super("div");
  ...
  public switchToCustom(formula?: string, domain?: string) {
    this.isCustomMode = true;
    if (formula) this.customFormula = formula;
    if (domain) this.customDomain = domain;
  ...
  private render() {
    ...
    for (const [key, preset] of Object.entries(EQUATION_PRESETS)) {
      ...
      if (!this.isCustomMode && this.spec.id === key) opt.selected = true;
    ...
    if (this.isCustomMode) customOpt.selected = true;
    ...
    if (this.isCustomMode) {
      ...
      setTimeout(() => {
        const applyBtn = builderPanel.querySelector("#eeCustomApplyBtn") as HTMLButtonElement;
        const lhsIn = builderPanel.querySelector("#eeCustomLhsInput") as HTMLInputElement;
        const domSel = builderPanel.querySelector("#eeCustomDomainSelect") as HTMLSelectElement;
        const rhsIn = builderPanel.querySelector("#eeCustomRhsSymbol") as HTMLInputElement;
