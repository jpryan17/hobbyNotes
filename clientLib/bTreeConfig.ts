...
export type BTreeInteractiveMode =
  | 'subtree'
  | 'simplicity'
  | 'order'
  | 'cut'
...
export type BTreeNodeLabelType = 'none' | 'sign' | 'dyadic' | 'birthday' | 'custom';

export interface BTreePalette {
  baseNode?: string;
  baseLink?: string;
  hoverNode?: string;
  selectedNode?: string;
  ...
  omegaStatePath?: string;
  omegaStateNode?: string;
  bg?: string;
  frame?: string;
  birthdayColors?: string[];
}
...
export interface BTreeConfig {
  ...
  arity?: number; // 0 for static, 1 or 2 for interactive
}
...
export const DEFAULT_PALETTE: BTreePalette = {
...
export const BTreePresets: Record<BTreeMode, Partial<BTreeConfig>> = {
  plain: {
    mode: 'plain',
    width: 900,
    height: 400,
