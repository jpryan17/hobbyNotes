...
export interface IBIDController {
  reset(...args: any[]): void;
  onScenarioChange?(scenario: BIDScenario): void;
  onUpdate?(): void;
}

export class SequentialEvidenceController implements IBIDController {
  scenario: BIDScenario;
  history: { step: number; eventName: string; hypotheses: BIDHypothesis[] }[] = [];
  currentStep = 0;

  constructor(scenarioKey: string = 'sensor') {
    this.scenario = JSON.parse(JSON.stringify(BID_SCENARIOS[scenarioKey] || BID_SCENARIOS.sensor));
    this.recordInitial();
  }

  private recordInitial() {
    this.history = [
      {
        step: 0,
  ...
  addEvidence(isPositive: boolean) {
    const prev = this.history[this.currentStep].hypotheses;
    const nextHypotheses: BIDHypothesis[] = [];
    ...
    if (totalJoint <= 0) totalJoint = 1e-9;
  ...
  getCurrentState(): { step: number; eventName: string; hypotheses: BIDHypothesis[] } {
    return this.history[this.currentStep];
  }
}

export class BetaBinomialController implements IBIDController {
  alpha = 1;
  beta = 1;
  heads = 0;
  tails = 0;
  ...
  getEffectiveAlphaBeta(): { a: number; b: number } {
    return {
  ...
  getTransectPoints(numPoints = 64): { x: number; density: number; weight: number }[] {
    const { a, b } = this.getEffectiveAlphaBeta();
    const points: { x: number; density: number; weight: number }[] = [];
