...
//
interface InTypeDesc{type:string,key:string,id:boolean,output:string}
//
...
const medSpace = '\u205f'
const largeSpace = '\u205f\u205f'
const InTypeDescList:InTypeDesc[]=
    [
...
export class SE extends SVGGrpElt{
    ...
    argNest=0
    key:string = ''
    inputMode:'L'|'P'|'A'|'Q'|'B' = 'L'
    expSide:'Front'|'Back' = 'Front'
    inList:string[] = []
    inScope:{var:string,lev:number}[] = []
    //
    constructor(public parent:SVGElt){
        super()
        SE.that = this
    ...
    layout(){
        const [width,height] = this.parent.getAA(['width','height']) as [string,string]   
    ...
    static handleInput(ev:KeyboardEvent){SE.that.handleInput(ev)}
    ...
    processKey(){    
         const desc = InTypeDescList.find(e=>this.key==e.key) as InTypeDesc
    ...
    canBeDigit(){
        if(this.inList.length>0){
            ...
            const desc = InTypeDescList.find(e=>e.type==prev) as InTypeDesc
            if (vars.includes(desc.type)){
                const pp = this.inList[this.inList.length-np-1]
                const ppDesc = InTypeDescList.find(e=>e.type==pp) as InTypeDesc
    ...
    fmt(){
        ...
        for (let i=0; i < this.inList.length; i++){
            ...
            if(this.isDigit(ex)){
                ...
                const desc = InTypeDescList.find(e=>e.type==ex) as InTypeDesc
                let tok = desc.output
                if (args.includes(ex)) {
