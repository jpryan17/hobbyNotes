export type BIDStaticMode =
  | 'transect'
  | 'filter'
  | 'mosaic'
  | 'treeProjection';

export type BIDInteractiveMode =
  | 'stateTree'
  | 'sequential'
  | 'oddsGauge'
  | 'continuous'
...
export type BIDMode = BIDStaticMode | BIDInteractiveMode;

export interface BIDHypothesis {
  id: string;
  name: string;
  prior: number;       // P(H)
...
export interface BIDScenario {
  ...
  description: string;
  evidenceName: string;
  hypotheses: BIDHypothesis[];
}

export interface BIDPalette {
  bg: string;
  frame: string;
  textDark: string;
  textMuted: string;
  ...
  transectGrid: string;
  hColors: string[];
  evidenceShade: string;
  posteriorGlow: string;
  treeBranch: string;
  highlight: string;
  accent: string;
}
...
export const DEFAULT_BID_PALETTE: BIDPalette = {
  bg: '#f8fafc',
  frame: '#94a3b8',
  textDark: '#1e293b',
  textMuted: '#64748b',
...
export const BID_SCENARIOS: Record<string, BIDScenario> = {
  medical: {
    id: 'medical',
...
export const BIDPresets: Record<BIDMode, Partial<BIDConfig>> = {
  transect: {
    mode: 'transect',
    width: 900,
    height: 440,
