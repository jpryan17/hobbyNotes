...
export type Rel = 'B' | 'A' | 'L' | 'R'
export type Loc = Quad[] | Rel
export type Range = [number,number]

type Span = {text:string,dx?:number,cb?:Function,w?:SVGTSpan}
type Line = {fontSize:number,pos:'L'|'C'|'R',topMargin:number,spans:Span[],w?:SVGText,len?:number}
...
export interface BN {
    ...
    color:{border:string,bg:string,fill:string,std:string,
            cb:string,over:string,inactive:string,busy:string,error:string},
    margin:number,
    lines:Line[]
}
...
export class Banner extends SVGElt{
    ...
    constructor(public bn:BN) {
        super('svg')
        const main = document.getElementById('main-slot') as HTMLElement
        ...
        bn.lines.forEach ((line,index) =>{
            ...
            let ww=0
            let sp=0
            let lineWidget = line.w as SVGText
            let lineLen = line.len as number
            if(line.pos == 'L'){
                xp = bn.margin
            } else if (line.pos == 'R'){
    ...
    processLine(line:Line) {
        const lineWidget = new SVGText() as SVGText
        line.w = lineWidget
