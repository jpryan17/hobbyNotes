...
import {SVGElt,SVGGrpElt, SelectableTextList, SelectableTextColors,
        SVGSelectableText, SVGText, textWidth, SVGTSpan} from './svgElt.js'
import {PXE, TreeNode} from './pxe.js'
import {ttd} from './ttd.js'

interface RadioButtonSet {widgets:SVGText[],choice:number}
//
interface Prop {exp:string, expFmt:string}
interface Rule {op:number,ie:number,side?:number}
interface SelectionTarget extends Rule {arg:number}
interface Arg {source:string, pos:number}
interface Inf extends Prop,Rule{args:Arg[],oc?:'O'|'C'|'A'}
interface Hyp {pos:number, cycle:number, h2?:string}
...
const colors:SelectableTextColors = {std:'blue',over:'purple',disabled:'grey',selected:'darkred'}
...
export class ProofBuilderDemo extends SVGElt{
    ...
    tempHypSVG!:SVGElt
    propFor:string = ''
    htl!:SelectableTextList
    itl!:SelectableTextList
    hList:Prop[] = []
    cVal!:Prop
    iList:Inf[]=[]
    conclusion!:SVGText
    selectionTarget!:SelectionTarget
    opWidgets:RadioButtonSet = {widgets:[],choice:-1}
    eiWidgets:RadioButtonSet = {widgets:[],choice:-1}
    sideWidgets:RadioButtonSet = {widgets:[],choice:-1}
    inferenceOKButton!:SVGSelectableText
    arg1!:Arg
    arg2!:Arg
    p1!:string
    p2!:string
    hypRef!:SVGSelectableText
    hypCls!:SVGSelectableText
    hypotheticals:Hyp[] = []
    closedHypotheticals:{s:number,e:number}[] = []
    //
    //
    constructor(public width:number,public height:number){
        ...
        this.htl = new SelectableTextList(cbfF,
                            {maxLineCharCnt:45,fontSize:listFontSize,maxDisplayLines:4,id:'H'})
        ...
        this.itl = new SelectableTextList(cbfF,
            {maxLineCharCnt:50,fontSize:listFontSize,maxDisplayLines:6,id:'I'})
    ...
    hSetList(){
        ...
        const hl:SVGText[] = this.hList.map((e,p)=>{
    ...
    iSetList(inf:Inf){
        ...
        //console.log(`boxX ${boxX} boxW ${boxW} sw ${sw} sc ${sc} endX ${endX}`)
        const il:SVGText[] = this.iList.map((inf,p)=>{
            const txt = new SVGText()
    ...
    inClosedHypothesis(p:number){
        this.closedHypotheticals.forEach(ch =>{
            if (p>=ch.s && p < ch.e) return true
        })
        return false
    }
    mainOp(prop:Prop){
    ...
    setText(text:string,grp?:SVGElt){
        const color = 'black'
        const fontSize = 18
        const txt = new SVGText()
        if (grp) {
    ...
    setRB(rbset:RadioButtonSet,text:string,grp?:SVGElt){
        const fontSize=18
        ...
        rb.elt.addEventListener('mouseover',(ev)=>{
            ...
            const op = Elt.wrapper(target) as SVGText
            op.setA('stroke',colors.over)
        })          
        rb.elt.addEventListener('mouseout',(ev)=>{
            if (rbset.choice == -1) {
                const target = ev.target as Element
                const sel = Elt.wrapper(target) as SVGText
    ...
    setSelectableText(fn:Function,text:string,grp?:SVGElt){
        const fontSize = 18
    ...
    cbf(tl:SelectableTextList,pos:number){
        ...
        if (opPos == 0 ) {  // and rules
            if (iePos == 0) { // and intro rule
                if(argPos == 0){
                    ...
                    const inf:Inf = {exp:newExp,expFmt:fmt,op:opPos,ie:iePos,args:[this.arg1,this.arg2]}
                    this.iSetList(inf)
                    this.endCk()
                }
                ...
                const subtree = (sidePos == 0)? tree.left as TreeNode : tree.right as TreeNode
                ...
                const inf:Inf = {exp:exp,expFmt:fmt,op:opPos,ie:iePos,side:sidePos,args:[this.arg1]}
                this.iSetList(inf)
                this.endCk()
            }
        } else if (opPos == 1 && iePos == 1){ // implication elimination rule
            const treeLeft = tree.left as TreeNode
            ...
            if (exp != ''){
                const treeRight = tree.right as TreeNode
            ...
            const sidePos = this.sideWidgets.choice
            const treeSide = (sidePos == 0)? tree.left as TreeNode : tree.right as TreeNode
            ...
            if (exp != ''){
                const treeOther = (sidePos == 0)? tree.right as TreeNode : tree.left as TreeNode
            ...
            if(iePos== 0){ // introduction
                ...
                const tl = tree.left as TreeNode
                const h1 = treeToExp(tl)
                const fmt = PXE.fmt(h1)
                const inf:Inf = {exp:h1,expFmt:fmt,op:opPos,ie:iePos,args:[],oc:'O'}
                this.iSetList(inf)
                const tr = tree.right as TreeNode
    ...
    peReturn(f:boolean,exp:string,expFmt:string){
        ...
        if (f){
            ...
            if(this.propFor == 'H'){
                ...
                if([1,2,4].includes(opPos) && iePos == 0){
                    const fmt = PXE.fmt(exp)
                    const inf:Inf = {exp:exp,expFmt:fmt,op:opPos,ie:iePos,args:[],oc:'O'}
                    this.iSetList(inf)
                    const hyp:Hyp = {pos:this.iList.length,cycle:-1}
                    this.hypotheticals.push(hyp)
                    this.handleHyp(hyp)
                }
                return
    ...
    contradiction(exp1:string,exp2:string){
    ...
    hypClsBtnHandler(){
        ...
        if (hypInf.op == 1){
            ...
            hypInf.oc = 'A'
            const exp = (hypInf.op == 2) ? this.iList[this.iList.length].exp : hyp.h2 as string
            const fmt = PXE.fmt(exp)
            const inf:Inf = {exp:exp,expFmt:fmt,op:hypInf.op,ie:hypInf.ie,args:[],oc:'O'}
            this.iSetList(inf)
    ...
    hypClose(){
        ...
        const fmt = PXE.fmt(exp)
        const newInf:Inf = {exp:exp,expFmt:fmt,op:hypInf.op,ie:hypInf.ie,args:[],oc:'C'}
        this.iSetList(inf)
        this.setTempHypState(false)
    }
    getBinExp(op:string,exp1:string,exp2:string){
        const rawExp = `[${exp1}]${op}[${exp2}]`
        const tree = ttd.bldTree(rawExp)
        return treeToExp(tree)
    }
    bracketArgIfNeeded(arg:Arg, node:TreeNode){
    ...
    setGrpState(grp:SVGGrpElt,state:boolean){
...
function cbfF(tl:SelectableTextList,pos:number){pb.cbf(tl,pos)}

function treeToExp(tree:TreeNode){
    ...
    if ( ! ['p','q','r','s'].includes(tt)) {
        ...
        let lse = (wl)? `[${ls}]` as string : ls as string
        let rse = (wr)? `[${rs}]` as string : rs as string
...
function salience(node:TreeNode){
...
function maxSal(node:TreeNode,max=0){
...
function needToWrap(node:TreeNode,sideIndicator:string){
    let rv = false
    const ns = salience(node)
    console.log(`node type ${node.nodeType} node salience ${ns}`)
    let side = (sideIndicator == 'L')? node.left as TreeNode: node.right as TreeNode
