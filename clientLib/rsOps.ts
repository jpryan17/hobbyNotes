...
export class RsOps {
    ...
    static addVals(inVals:string[], val:string){
        if(RsOps.bailed)
            return []
        let vals:string[]=[]
    ...
    static multiplyVals(X:string,Y:string,s1:string[],s2:string[]) :string[]{
        if(RsOps.bailed)
            return []
        let vals:string[]=[]
        for (let x of s1) {
            for (let y of s2) {
                vals.push(RsOps.add(RsOps.add(RsOps.multiply(x,Y),RsOps.multiply(X,y)),
                                    RsOps.invertVal(RsOps.multiply(x,y))))
            }
        }
        return vals
    }
    static invertVal(val:string){
        let rv = ''
    ...
    static simplerSides(exp:string){
        let ls:string[]=[]
        let rs:string[]=[]
    ...
    static extremeVal(dir:number, vals:string[]) :string{
        if (vals.length==0)
