...
export class PXE extends SVGGrpElt {
  ...
  static predicateChars = "pqrsmvklud";
  static negChar = "n";
  static andChar = "a";
  static orChar = "o";
  static implyChar = "i";
  ...
  displayState: "Building" | "Valid" = "Building";

  constructor(public parent: PXEParent) {
    super();
    PXE.that = this;
  ...
  layout() {
    const [width, height] = this.parent.getAA(["width", "height"]) as [
      string,
      string,
    ];
  ...
  backspace(): void {
    if (this.exp.length > 0) {
      if (this.exp[this.exp.length - 1] == PXE.lb) this.nl--;
      if (this.exp[this.exp.length - 1] == PXE.rb) this.nl++;
