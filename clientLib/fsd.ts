...
export class FSD extends PXEParent {
  ...
  renderStage4Table() {
    ...
    predCodes.forEach((code, idx) => {
      ...
      th.setAA([
        "style",
        `border: 1px solid #ccc; padding: 8px; font-size: 13px; font-family: monospace; font-weight: bold; cursor: pointer; background: ${
          isSelected ? "#e2f0fb" : "#f8f9fa"
        }; color: #0056b3;`
      ]);
      th.setV(`${p.name} 🔍`);
    ...
    expCols.forEach((col) => {
      ...
      th.setAA([
        "style",
        `border: 1px solid #ccc; padding: 8px; font-size: 13px; font-family: monospace; font-weight: bold; color: ${
          isOp ? "#d9534f" : "#333"
        }; background: #ffffff;`
      ]);
      th.setV(col.label);
    ...
    predCodes.forEach((code, idx) => {
      ...
      td.setAA([
        "style",
        `border: 1px solid ${isSelected ? "#007bff" : "#ccc"}; padding: 10px; font-weight: bold; font-size: 16px; cursor: pointer; color: ${
          p.val ? "#155724" : "#721c24"
        }; background: ${p.val ? "#d4edda" : "#f8d7da"};`
      ]);
      td.setV(p.val ? "T" : "F");
    ...
    expCols.forEach((col) => {
      ...
      td.setAA([
        "style",
        `border: 1px solid #ccc; padding: 10px; font-weight: bold; font-size: 16px; color: ${
          col.val ? "#155724" : "#721c24"
        }; background: ${col.val ? "#d4edda" : "#f8d7da"}; ${
          isOp ? "border: 2px solid #d9534f;" : ""
        }`
      ]);
      td.setV(col.val ? "T" : "F");
      trBody.append(td);
