...
export class FSD extends PXEParent {
  ...
  renderStage4Table() {
    ...
    const predTokens: { code: string; name: string }[] = [];
    if (this.pxe.exp.includes("p")) predTokens.push({ code: "p", name: "PG5" });
    if (this.pxe.exp.includes("q")) predTokens.push({ code: "q", name: "PL10" });
    if (this.pxe.exp.includes("r")) predTokens.push({ code: "r", name: "PG" });
    if (this.pxe.exp.includes("s")) predTokens.push({ code: "s", name: "PL" });

    // Build Table Header Columns (Quantified Predicates + Compound Statement)
    const columns: { label: string; isPred: boolean; predCode?: string }[] = [];
    ...
    columns.forEach((col, idx) => {
      ...
      th.setAA([
        "style",
        `border: 1px solid ${borderColor}; padding: 10px; font-size: 13px; font-family: monospace; font-weight: bold; cursor: pointer; background: ${bg}; color: ${
          col.isPred ? "#0056b3" : "#212529"
    ...
    columns.forEach((col, idx) => {
      ...
      td.setAA([
        "style",
        `border: 1px solid ${isSelected ? "#007bff" : "#ccc"}; padding: 12px; font-weight: bold; font-size: 18px; cursor: pointer; color: ${
          val ? "#155724" : "#721c24"
