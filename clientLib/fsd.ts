...
export type DomainType = "S" | "P(S)";

export interface SlotInfo {
  predCode: string; // 'p'|'q'|'r'|'s'|'m'
  predName: string; // 'PG5'|'PL10'|'PG'|'PL'|'∈'
  slotIndex: number; // 0 or 1
  domainType: DomainType; // 'S' (element) or 'P(S)' (subset)
  assignedVar?: string; // 'x₁', 'y₁', etc.
}

export interface QuantifierBinding {
  quantifier: "∀" | "∃";
  variable: string; // 'x₁', 'y₁', etc.
  domainType: DomainType; // 'S' or 'P(S)'
}
...
export class FSD extends PXEParent {
  ...
  applyQuantifier(qSymbol: "∀" | "∃") {
    ...
    if (unquantifiedVars.length > 0) {
      ...
      if (targetVar) {
        ...
        const dType: DomainType = slot ? slot.domainType : (targetVar.startsWith("y") ? "P(S)" : "S");
  ...
  renderStage4Table() {
    ...
    if (this.pxe.exp.includes("m")) predMap["m"] = { name: "∈", val: this.evaluatePredicate("∈") };
