...
export function setExp(id:string) :string{
    if(id=='leftPerp' || id=='rightPerp') return WU.undetermined
    let bd = Number(id.substring(1,2))
    let lp = Number(id.substring(2))
    return bdLpToExp(bd,lp)
}
export function bdLpToExp(bd:number,lp:number){
...
export function setVal(exp:string){
    if (exp.length==0) 
        return ' [ ] '
...
export function keyToBdLp(key:string){
    const bd = +key.substring(1,2)
    const lp = +key.substring(2)
    return [bd,lp]  
}
export function keyToExp(key:string){
    const [bd,lp] = keyToBdLp(key)
    return bdLpToExp(bd,lp)
}
export function expToBdPos(exp: string) :[number,number]{
    let bd = exp.length
    let len = Math.pow(2,bd)
    let slen = len/2
    for (let i=0;i<bd;i++){
...
export function expToId(exp: string){
    const [bd,pos] = expToBdPos(exp)
    return `K${bd}${pos}`
}
export function compareExps(r1:string, r2:string){
    for(let i=0;i<Math.max(r1.length,r2.length);i++){
        if(i>=r1.length){
            if(i>=r2.length) return 0
            if(r2[i]==WU.plus) return 1
            return -1
        } else if (r1[i]==WU.plus){
            if(i>=r2.length) return -1
            if(r2[i] == WU.minus) return -1
        } else {
            if(i>=r2.length) return 1
            if(r2[i]==WU.plus) return 1
        }
...
function setMinLength(reals:string[]){
    let len= reals[0].length
    reals.forEach((r)=>{if (r.length < len) len = r.length})
    return len 
}
function setBetweenCandidates(r1:string,r2:string){
...
function uniqueBetweens(reals:string[]){
    let uniques:string[] = []
    for (let i=0;i<reals.length;i++){
        if (! uniques.includes(reals[i])) uniques.push(reals[i])
    }
    return uniques
}
function findSimpleOf(reals:string[]){
    const rc = reals.length
    const minLen = setMinLength(reals)
    let simple = ''
    for(let i=0;i<minLen;i++){
        for (let j=1;j<rc;j++){
            if (reals[0][i] != reals[j][i]) return simple
        }
        simple = simple.concat(reals[0][i])
    }
    return simple
}
export function findCut(ra:string,rb:string) {
...
export function setCut(lx:string,rx:string){
