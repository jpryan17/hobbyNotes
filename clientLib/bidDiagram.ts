...
export class BIDDiagram extends SVGElt {
  ...
  constructor(configOrMode: BIDConfig | BIDMode) {
    super('svg');
    if (typeof configOrMode === 'string') {
      const preset = BIDPresets[configOrMode] || {};
      this.config = { mode: configOrMode, ...preset } as BIDConfig;
    } else {
  ...
  scaleToWidth(usableWidth: number) {
  ...
  private renderStateTreeView() {
    ...
    const getNodePos = (d: number, pos: number): [number, number] => {
    ...
    const getOutcomeStr = (d: number, pos: number): string => {
      if (d === 0) return '';
      let s = '';
    ...
    const isOnAscendingPath = (d: number, pos: number): boolean => {
      if (d > selD) return false;
      const shift = selD - d;
      return (selPos >> shift) === pos;
    };

    // Helper to check if a node (d, pos) is in the descendant subtree cone above (selD, selPos)
    const isInSubtreeCone = (d: number, pos: number): boolean => {
      if (d < selD) return false;
      const shift = d - selD;
  ...
  private renderFilterView() {
    ...
    curX = tx;
    let totalJoint = 0;
    const jointData: { h: any; joint: number; w: number; activeW: number }[] = [];
  ...
  // =========================================================================
  private renderTreeProjectionView() {
    const g = this.contentGroup;
  ...
  private renderSequentialView() {
    ...
    history.forEach((rec, stepIdx) => {
      ...
      if (curY > 380) return; // Limit visual overflow
  ...
  // =========================================================================
  private addHeader(g: SVGGrpElt, title: string, y: number) {
    const t = new SVGText();
  ...
  private addText(g: SVGGrpElt, text: string, x: number, y: number, size = 12, color = '#1e293b', anchor = 'start', bold = false) {
    const t = new SVGText();
    t.setAA(['x', x, 'y', y, 'font-size', size, 'fill', color, 'text-anchor', anchor]);
    if (bold) t.setA('font-weight', 'bold');
    t.setV(text);
    g.append(t);
  }

  private drawRect(g: SVGGrpElt, x: number, y: number, w: number, h: number, fill: string, stroke = '#cbd5e1', opacity = 1, rx = 0) {
    if (w <= 0 || h <= 0) return;
    const r = new SVGElt('rect');
    r.setAA(['x', x, 'y', y, 'width', w, 'height', h, 'fill', fill, 'stroke', stroke, 'stroke-width', 1, 'opacity', opacity]);
    if (rx > 0) r.setA('rx', rx);
    g.append(r);
  }

  private drawLine(g: SVGGrpElt, x1: number, y1: number, x2: number, y2: number, stroke = '#94a3b8', width = 1.5) {
    const l = new SVGElt('line');
    l.setAA(['x1', x1, 'y1', y1, 'x2', x2, 'y2', y2, 'stroke', stroke, 'stroke-width', width]);
    g.append(l);
  }

  private drawArrowDown(g: SVGGrpElt, x: number, y: number, label: string) {
  ...
  private drawPan(g: SVGGrpElt, x: number, y: number, label: string, prob: string, color: string) {
    const line = new SVGElt('line');
