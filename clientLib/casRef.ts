...
export class CasRef extends HTMLElement {
  ...
  connectedCallback(): void {
    ...
    this.addEventListener('click', (e) => {
      ...
      const matchedStatement = FS_CATALOG.formalStatements.find(
        (s) =>
      ...
      if (scaffoldKey || matchedStatement) {
        ...
        const cardTitle =
          this.innerText.replace(/^<<CAS:\s*/, '').replace(/>>$/, '').trim() ||
        ...
        Nav.fo.setA(
          'style',
          'overflow-y:auto;overflow-x:hidden;box-sizing:border-box;padding:16px 20px 80px 20px;'
        );

        const scrollWrap = new Elt('div');
        scrollWrap.setA(
          'style',
          'width:100%;min-height:100%;box-sizing:border-box;display:flow-root;padding-bottom:60px;'
        );
        scrollWrap.append(
