...
export class SVGGrpElt extends SVGElt {
    ...
    gmove(x, y) {
        const [mx, my] = [x - this.x, y - this.y];
        this.setA('transform', `translate(${mx},${my})`);
        this.x = x;
        this.y = y;
    }
}
...
export class SVGBanner extends SVGGrpElt {
    constructor(id, lines, overrides) {
        super(id);
        this.size = { borderWidth: 2 };
        this.color = { border: 'darkblue', fill: 'lightBlue' };
        this.margin = { horz: 40, vert: 30, betweenLines: 15 };
        this.center = [0, 0];
        handleOverrides(this, overrides);
        this.rect = new SVGElt('rect', id.concat('-shape'));
        let maxWidth = 0;
        let sumHeights = 0;
        let lineWidgets = [];
        lines.forEach(line => {
            const [text, size, color] = line;
            const lineW = new SVGText();
            lineW.setV(text);
            lineW.setAA(['font-size', size, 'fill', color]);
            const [w, _] = lineW.eltWH();
            sumHeights += size;
            if (w > maxWidth)
                maxWidth = w;
            lineWidgets.push([lineW, w, size]);
        });
        const rw = maxWidth + 2 * this.margin.horz + this.size.borderWidth;
        const rh = sumHeights + (lines.length - 1) * this.margin.betweenLines + 2 * this.margin.vert;
        this.rect.setAA(['x', 0, 'y', 0, 'width', rw, 'height', rh, 'fill', this.color.fill,
            'stroke', this.color.border, 'stroke-width', this.size.borderWidth]);
        this.elt.appendChild(this.rect.elt);
        let sum = 0;
        for (let i = 0; i < lineWidgets.length; i++) {
            const [lineW, w, h] = lineWidgets[i];
            const xp = 1 / 2 * rw - 1 / 2 * w;
            const yp = this.margin.vert + sum + i * this.margin.betweenLines + .75 * h;
            sum += h;
            lineW.setAA(['x', xp, 'y', yp]);
            this.elt.appendChild(lineW.elt);
        }
    }
}
...
export class Diagram extends SVGElt {
    ...
    setCircle(cx, cy, radius, fill, id) {
        ...
        cc.setAA(['cx', cx, 'cy', cy, 'r', radius, 'fill', fill]);
...
export class NavLine {
    constructor(overrides) {
        handleOverrides(this, overrides);
        NavLine.width = window.innerWidth - NavLine.margin.line;
        const navSlot = document.getElementById('nav-slot');
        NavLine.navW = new Elt({ elt: navSlot });
        NavLine.navW.setAA(['width', NavLine.width, 'height', NavLine.height, 'style',
            'position:fixed;top:0']);
        NavLine.diagram = new Diagram({});
        NavLine.diagram.setAA(['id', 'nav', 'width', NavLine.width, 'height', NavLine.height,
            'style', `background-color:${NavLine.color.bg}`]);
        NavLine.navW.elt.appendChild(NavLine.diagram.elt);
        const mainSlot = document.getElementById('main-slot');
        NavLine.mainW = new Elt({ elt: mainSlot });
        NavLine.mainW.setA('style', 'margin-top:23');
        NavLine.leftX = 0;
    }
    static add(nav, fs, cb) {
        NavLine.diagram.elt.appendChild(nav.elt);
        nav.setAA(['x', NavLine.leftX + this.margin.std, 'font-size', fs,
            'y', 1 / 2 * (20 - fs) + .75 * fs, 'stroke', NavLine.color.std]);
        nav.elt.addEventListener('click', () => { cb(); });
        nav.elt.addEventListener('mouseover', () => { nav.setA('stroke', NavLine.color.over); });
        nav.elt.addEventListener('mouseout', () => { nav.setA('stroke', NavLine.color.std); });
        const nv = nav.elt;
        const bb = nv.getBBox();
        NavLine.leftX = bb.x + bb.width;
    }
    static remove(nav) {
        NavLine.diagram.elt.removeChild(nav.elt);
        NavLine.leftX = 0;
        const remaining = Array.from(NavLine.diagram.elt.children);
        remaining.forEach(nv => {
            const x = NavLine.leftX + NavLine.margin.std;
            nv.setAttribute('x', x.toString());
            const nvg = nv;
            const bb = nvg.getBBox();
            NavLine.leftX = x + bb.width;
        });
    }
    static clear() {
        NavLine.diagram.elt.innerHTML = '';
        NavLine.leftX = 0;
    }
}
NavLine.color = { bg: 'beige', std: 'darkblue', over: 'purple' };
NavLine.margin = { std: 25, line: 30 };
NavLine.height = 22;
