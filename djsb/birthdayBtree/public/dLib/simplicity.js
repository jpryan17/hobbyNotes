import { WU, setExpression, setVal, compareExps, expToId } from './exputils.js';
...
export class SimplicityDiagram extends SVGElt {
    ...
    constructor() {
        ...
        that = this;
        this.tree = new InteractiveTree(this, 'simplicity', this.treeWidth, this.treeHeight, this.treeMaxBD, this.treeNodeSize, 1, processSelection);
        this.tree.setAA(['x', 20, 'y', 20]);
        this.tree.w = this.treeWidth;
        this.tree.h = this.treeHeight + 125;
        this.cmd = new SVGText();
        this.elt.appendChild(this.cmd.elt);
        this.cmd.setAA(['x', 25, 'y', this.tree.h - 40, 'font-size', '20', 'stroke', 'cadetblue']);
        this.cmd.setV('select node');
        this.box = new SVGText();
        this.box.setAA(['x', 25, 'y', this.tree.h + 15, 'font-size', '20', 'stroke', 'black', 'visibility', 'hidden']);
    ...
    updateOutputBox() {
        ...
        this.selected.setV(setVal(setExpression(this.tree.wasVisited[0])));
    ...
    simplerNodes() {
        let nodeExp = setExpression(this.tree.wasVisited[0]);
        let nodes = [];
    ...
    simplerLeftRightNodes() {
        let keyExp = setExpression(this.tree.wasVisited[0]);
        let nodes = this.simplerNodes();
        let leftNodes = [];
        let rightNodes = [];
        for (let node of nodes) {
            if (compareExps(setExpression(node), keyExp) == 1)
                leftNodes.push(node);
            else
    ...
    labelList(nodes) {
        if (nodes == undefined || nodes.length == 0)
            ...
            let rv = [];
            for (let node of nodes)
                rv.push(setVal(setExpression(node)));
